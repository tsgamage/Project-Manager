          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              ...
              <div className="relative">
                ...
                >
                  {showPassword ? (
                    <svg className="h-5 w-5 text-stone-400 hover:text-stone-600 dark:hover:text-stone-300" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div>
              <label htmlFor="confirmPassword" className="block text-sm font-medium text-stone-700 dark:text-stone-300 mb-2">
              ...
              <div className="relative">
                <input
                  ...
                  onChange={handleInputChange}
                  className={`w-full px-4 py-3 pr-12 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-stone-700 text-stone-900 dark:text-stone-100 placeholder-stone-500 dark:placeholder-stone-400 transition-colors ${
                    errors.confirmPassword ? 'border-red-500' : 'border-stone-300 dark:border-stone-600'
                  }`}
            ...
            <div className="bg-stone-50 dark:bg-stone-700/50 rounded-lg p-4">
              ...
              <ul className="text-xs text-stone-600 dark:text-stone-400 space-y-1">
                <li className={`flex items-center ${formData.password.length >= 8 ? 'text-green-600 dark:text-green-400' : ''}`}>
                  <svg className={`w-3 h-3 mr-2 ${formData.password.length >= 8 ? 'text-green-600 dark:text-green-400' : 'text-stone-400'}`} fill="currentColor" viewBox="0 0 20 20">
                ...
                <li className={`flex items-center ${/(?=.*[a-z])/.test(formData.password) ? 'text-green-600 dark:text-green-400' : ''}`}>
                  <svg className={`w-3 h-3 mr-2 ${/(?=.*[a-z])/.test(formData.password) ? 'text-green-600 dark:text-green-400' : 'text-stone-400'}`} fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                ...
                <li className={`flex items-center ${/(?=.*\d)/.test(formData.password) ? 'text-green-600 dark:text-green-400' : ''}`}>
                  <svg className={`w-3 h-3 mr-2 ${/(?=.*\d)/.test(formData.password) ? 'text-green-600 dark:text-green-400' : 'text-stone-400'}`} fill="currentColor" viewBox="0 0 20 20">
                    <path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" />
                  </svg>
                  Contains number
                </li>
