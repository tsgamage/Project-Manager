...
export default function NavigationTabs({ onNavigationClick, activeTab }) {
    ...
    return <div className="px-4 sm:px-6 lg:px-8 mb-8">
        <div className="grid grid-cols-2 sm:flex sm:flex-wrap gap-2">
            {tabs.map((tab) => (
                <button
                    key={tab.id}
                    onClick={() => onNavigationClick(tab.id)}
                    className={`flex cursor-pointer items-center justify-center gap-2 px-3 sm:px-4 py-3 rounded-xl font-medium transition-all duration-300 text-sm sm:text-base ${activeTab === tab.id
