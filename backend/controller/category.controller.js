...
export async function getProjectCategories(req, res) {
  ...
  try {
    ...
    res
      .status(500)
      .json({ success: false, message: err.message || "Cannot get project categories" });
  }
}
...
export async function updateMemberCategory(req, res) {
  ...
  try {
    const category = await MemberCategory.findByIdAndUpdate(categoryID, categoryData);
...
export async function updateProjectCategory(req, res) {
  ...
  try {
    const category = await ProjectCategory.findByIdAndUpdate(categoryID, categoryData);
    if (!category) return res.status(404).json({ success: false, message: "Category not found" });
    res
      .status(200)
      .json({ success: true, message: "Category updated successfully", data: category });
